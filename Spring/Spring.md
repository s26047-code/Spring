...
영어 풀이의 의미 그대로 "제어의 역전" 이라는 뜻으로,  
**객체의 생성, 관리, 흐름을 개발자가 아닌 외부(프레임워크)**가 해주는 관리 시스템이다.
...
스프링에서는 공통적으로 사용되는 기능들을 효율적으로 관리하기 위한 기능이다. 이때 AOP의 뜻을 직역하면 관점 지향 프로그래밍이 된다.
...
## - PSA (Portable Service Abstraction)
 PSA를 직역한 뜻은 이식 가능한 서비스 추상화이다. 스프링에서 제공하는 다양한 기술들을 추상화해 개발자가 쉽게 사용하는 인터페이스를 의미함.
...
# 어노테이션 (Annotation)
자바 등 프로그래밍 언어에서 소스 코드에 코드에 대한 정보를 추가하여 컴파일러나 런타임 환경에 특별한 정보를 전달하는 @ 형태의 표식이다. 주로 코드 가독성 향상, 자동 코드 생성, Spring의 설정 단순화 등 빌드 시 문법 체크 등에 사용된다.
...
어노테이션은 크게 세 가지의 종류로 구분된다고 한다. 그 종류로는 자바에서 기본적으로 제공하는 표준 어노테이션, 어노테이션을 정의하는 데 사용되는 메타 어노테이션, 마지막으로 사용자 어노테이션이 있다.
...
ㄴ> 사용자 정의 어노테이션을 만들기 위해선 @interface가 필요하다고 한다. 
...
+ json API란, 데이터를 JSON 형태로 주고받는 웹 통신 방식이다.  
